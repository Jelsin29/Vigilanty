...
func ConfigYAMLForPreset(preset string) (string, error) {
	...
	case "", "default", "go":
		content = goPresetConfigYAML()
	case "node":
		content = nodePresetConfigYAML()
	case "typescript":
		content = typescriptPresetConfigYAML()
	case "python":
		content = pythonPresetConfigYAML()
	case "rust":
		content = rustPresetConfigYAML()
	case "java":
		content = javaPresetConfigYAML()
	case "dotnet":
		content = dotnetPresetConfigYAML()
	case "ruby":
		content = rubyPresetConfigYAML()
	case "swift":
		content = swiftPresetConfigYAML()
	case "php":
		content = phpPresetConfigYAML()
	case "generic":
		content = genericPresetConfigYAML()
	default:
	...
	return withPresetGuidance(strings.ToLower(strings.TrimSpace(preset)), content), nil
}

func withPresetGuidance(preset string, content string) string {
	name := preset
	if name == "" || name == "default" {
		name = "go"
	}

	return fmt.Sprintf(`# Generated by vigilanty init --preset %s
# Review the starter steps below, then run:
#   vigilanty install
#   vigilanty run
#   vigilanty run --json
